public class Jeu : JeuProtocol{
  ...
  public required init()
  {
    ...
    self.pieces.append(Piece(numero : 1, nom : "koropokkuru", position : position0, joueur : 1))
    self.pieces.append(Piece(numero : 2, nom : "koropokkuru", position : position1, joueur : 2))
    self.pieces.append(Piece(numero : 3, nom : "kodama", position : position2, joueur : 1))
    self.pieces.append(Piece(numero : 4, nom : "kodama", position : position3, joueur : 2))
    self.pieces.append(Piece(numero : 5, nom : "kitsune", position : position4, joueur : 1))
    self.pieces.append(Piece(numero : 6, nom : "kitsune", position : position5, joueur : 2))
    self.pieces.append(Piece(numero : 7, nom : "tanuki", position : position6, joueur : 1))
    self.pieces.append(Piece(numero : 8, nom : "tanuki", position : position7, joueur : 2))
  }
  ...
  public func makeIterator() -> IteratorJeu{
  ...
  public func getPiece (position : Position) -> Piece?{
    var p2 : Piece?
    if position.getB(){
      for p in self{
  ...
  public func estOccupeeAllie (position : Position, joueur : Int) -> Bool{
    var b : Bool = false
  ...
  public func estOccupeeEnnemi (position : Position, joueur : Int) -> Bool{
    var b : Bool = false
    if position.getB(){//position occupée
      for p in self{
        if p.getPosition().getX()==position.getX() && p.getPosition().getY()==position.getY(){//la piece a la bonne position
...
public struct IteratorJeu : IteratorProtocol{
